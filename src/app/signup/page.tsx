...
const translations = {
  en: {
    title: 'Join BatataBotato',
    subtitle: 'Start your custom spud journey',
    username: 'User Name',
    password: 'Password',
    signup: 'Sign Up',
    signingUp: 'Planting...',
    alreadyHave: 'Already have a spud?',
    login: 'Login',
    errorTaken: 'This User Name is already taken.',
    errorGeneric: 'Something went wrong.',
    placeholderUsername: 'CoolBatata123',
  },
  ar: {
    title: 'انضم إلى بطاطا بوتاتو',
    subtitle: 'ابدأ رحلتك مع حبة البطاطس الخاصة بك',
    username: 'اسم المستخدم',
    password: 'كلمة المرور',
    signup: 'إنشاء حساب',
    signingUp: 'جاري الزراعة...',
    alreadyHave: 'لديك حساب بالفعل؟',
    login: 'تسجيل الدخول',
    errorTaken: 'اسم المستخدم هذا محجوز بالفعل.',
...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    // Step 1: Create user via server-side API (no email confirmation)
    ...
    // Step 2: Sign in client-side to get the session
  ...
  if (loading) {
    return (
      <div dir={lang === 'ar' ? 'rtl' : 'ltr'} style={{
        ...
        <p style={{ fontWeight: '600', color: '#a0a0a0', fontSize: '1.1rem' }}>{t.signingUp}</p>
      </div>
    );
  }
    ...
    <div dir={lang === 'ar' ? 'rtl' : 'ltr'} style={{
      ...
      {loading && (
        <div style={{
          ...
          <p style={{ marginTop: '16px', fontWeight: '600' }}>{t.signingUp}</p>
        </div>
      )}
      ...
      <div className="glass" style={{
        ...
        <form onSubmit={handleSignup} style={{ textAlign: lang === 'ar' ? 'right' : 'left' }}>
          ...
          <button
            ...
            disabled={loading}
          >
            {loading ? <Loader2 className="animate-spin" size={20} /> : <>{t.signup} <ArrowRight size={18} style={{ transform: lang === 'ar' ? 'rotate(180deg)' : 'none' }} /></>}
          </button>
        </form>

        <p style={{ marginTop: '24px', color: '#a0a0a0', fontSize: '0.9rem' }}>
          {t.alreadyHave} <Link href="/login" style={{ color: 'var(--primary)', textDecoration: 'none', fontWeight: '600' }}>{t.login}</Link>
        </p>
      </div>
    </div>
  );
